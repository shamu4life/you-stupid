...
- `public/index.html` — the youtooz product-page clone + embedded idiot engine. **Hand-authored** (not from the build script). Matches youtooz's real look: self-hosted **Luckiest Guy** (display: title, section headings, countdown digits, buttons) + **Inter** (body) + **Material Icons** (codepoint glyphs) from `public/fonts/`, the real youtooz logo SVG, white/black palette and rounded panels. Product data (title, `$29.99 USD`, description, pre-order line, frozen countdown) mirrors the real page; `Add to cart` calls `detonate()`.
